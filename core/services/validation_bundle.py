...
class ValidationBundleError(RuntimeError):
    """Error controlado al crear el bundle de validación."""
...
class ValidationBundleService:
    def build_bundle(self, analysis: ProjectAnalysis) -> ValidationBundle:
        ...
        try:
            ...
            bundle_path = workspace_root / "validation-bundle.zip"
    ...
    def _enforce_size_limit(self, bundle_size_bytes: int) -> None:
        ...
        if bundle_size_bytes > max_bytes:
            raise ValidationBundleError(
                f"El bundle de validación excede el límite de {settings.AUTODOCKER_VALIDATION_MAX_BUNDLE_MB} MB."
            )
