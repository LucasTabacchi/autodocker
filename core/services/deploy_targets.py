from __future__ import annotations

from dataclasses import asdict, dataclass, field

from core.services.contracts import DetectionResult, GeneratedArtifactSpec


@dataclass(slots=True)
class DeployReport:
    summary: str
    targets: list[str]
    generated_paths: list[str]
    maturity: str = "bootstrap"
    follow_up: list[str] = field(default_factory=list)

    def to_dict(self) -> dict:
        return asdict(self)


class DeployTargetArtifactService:
    def generate(self, detection: DetectionResult, profile: str) -> tuple[list[GeneratedArtifactSpec], DeployReport]:
        render_path = "render.yaml"
        k8s_path = "deploy/kubernetes/app.yaml"
        artifacts = [
            GeneratedArtifactSpec(
                kind="deploy",
                path=render_path,
                description="Blueprint base para desplegar el proyecto como servicio web.",
                content=self._render_blueprint(detection, profile),
            ),
            GeneratedArtifactSpec(
                kind="deploy",
                path=k8s_path,
                description="Manifiesto base de Deployment + Service para Kubernetes.",
                content=self._kubernetes_manifest(detection, profile),
            ),
        ]
        report = DeployReport(
            summary="Targets bootstrap generados para PaaS y Kubernetes.",
            targets=["render", "kubernetes"],
            generated_paths=[render_path, k8s_path],
            follow_up=[
                "Ajustar imagen, secrets, recursos y healthchecks según el stack real.",
                "Validar estrategia de rollout, observabilidad y storage antes de usar en producción.",
            ],
        )
        return artifacts, report

    def _render_blueprint(self, detection: DetectionResult, profile: str) -> str:
        component = detection.primary_component()
        port = component.probable_ports[0] if component and component.probable_ports else 8000
        pre_deploy_command = ""
        if component and component.framework == "Django":
            pre_deploy_command = "python manage.py migrate --noinput"
        return "\n".join(
            [
                "# Bootstrap scaffold generated by AutoDocker.",
                "# Review runtime plan, secrets, persistent disks and Dockerfile before production use.",
                "services:",
                "  - type: web",
                f"    name: {detection.project_name.lower().replace(' ', '-')}",
                "    runtime: docker",
                "    dockerfilePath: ./Dockerfile" if not component or component.path == "." else f"    dockerfilePath: ./{component.path}/Dockerfile",
                f"    dockerContext: {component.path if component and component.path != '.' else '.'}",
                "    plan: starter",
                "    autoDeploy: true",
                "    healthCheckPath: /health/",
                "    envVars:",
                "      - key: AUTODOCKER_PROFILE",
                f"        value: {profile}",
                "      - key: PORT",
                f"        value: \"{port}\"",
            ]
            + (
                [
                    f"    preDeployCommand: {pre_deploy_command}",
                ]
                if pre_deploy_command
                else []
            )
        )

    def _kubernetes_manifest(self, detection: DetectionResult, profile: str) -> str:
        component = detection.primary_component()
        app_name = detection.project_name.lower().replace(" ", "-")
        port = component.probable_ports[0] if component and component.probable_ports else 8000
        return "\n".join(
            [
                "# Bootstrap scaffold generated by AutoDocker.",
                "# Review image, secrets, resources, probes and rollout policy before production use.",
                "apiVersion: apps/v1",
                "kind: Deployment",
                "metadata:",
                f"  name: {app_name}",
                "spec:",
                "  replicas: 1",
                "  selector:",
                "    matchLabels:",
                f"      app: {app_name}",
                "  template:",
                "    metadata:",
                "      labels:",
                f"        app: {app_name}",
                "    spec:",
                "      containers:",
                "        - name: app",
                "          image: your-registry/replace-me:latest",
                "          ports:",
                f"            - containerPort: {port}",
                "          env:",
                "            - name: AUTODOCKER_PROFILE",
                f"              value: {profile}",
                "          readinessProbe:",
                "            httpGet:",
                "              path: /",
                f"              port: {port}",
                "            initialDelaySeconds: 10",
                "            periodSeconds: 15",
                "---",
                "apiVersion: v1",
                "kind: Service",
                "metadata:",
                f"  name: {app_name}",
                "spec:",
                "  selector:",
                f"    app: {app_name}",
                "  ports:",
                f"    - port: {port}",
                f"      targetPort: {port}",
                "  type: ClusterIP",
            ]
        )
